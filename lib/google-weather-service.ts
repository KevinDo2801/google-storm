...
export class GoogleWeatherService {
  ...
  async getCurrentWeather(lat: number, lng: number): Promise<WeatherResponse> {
    ...
    try {
      ...
      console.error('Error fetching current weather, using fallback:', error);
      return this.getMockCurrentWeather(lat, lng);
    }
  }
  ...
  /**
   * Get mock current weather data as fallback
   */
  private getMockCurrentWeather(lat: number, lng: number): WeatherResponse {
    const now = new Date();
    const hour = now.getHours();
   ...
   */
  private getMockForecast(lat: number, lng: number): WeatherForecastResponse {
    const now = new Date();
    const forecast: WeatherForecast[] = [];
    const isHurricaneSeason = this.isHurricaneSeason(now.toISOString());
    ...
    for (let i = 0; i < 5; i++) {
      ...
      forecast.push({
        ...
        windSpeed,
        humidity: Math.round(60 + Math.random() * 30), // 60-90%
        isHurricaneRisk: this.isHurricaneRisk(windSpeed, precipitation, date.toISOString()),
      });
    }
